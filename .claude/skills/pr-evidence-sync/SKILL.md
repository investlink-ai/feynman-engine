...
- [x] `AgentRiskManager::evaluate` returns `Err` when stop loss missing
  — `crates/agent-risk/src/lib.rs:42`, test: `test_stop_loss_required` ✓
- [x] All 7 MVP checks present and tested
  — `cargo test -p agent-risk` → 7 tests pass
